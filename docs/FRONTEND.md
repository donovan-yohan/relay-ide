...
- **Implemented now:** session/file `WorkspaceTab` layout, optional session `nodeId`, node-aware terminal creation and PTY routing, active-tab-derived utility rail context, guarded rail states for remote and non-git tabs, six-layer identity helpers, persisted IA Workspace grouping, workspace topic APIs, topic-room sidebar shell, and bounded CLI file RPC.
- **Compatibility still present:** the right rail follows active tab/session context, while App/PR/dashboard/session creation still use `activeRepoPath`, `repoPath`, and `worktreePath` where the flow is git-specific or not yet lifted to product IA nouns.
...
- **Render swap:** `Sidebar.tsx` mounts `<ViewSpineTree>` (`frontend/src/components/ViewSpineTree.tsx`) when the flag is ON. The default path is the WorkspaceTopic task-room sidebar shell.
...
| `Sidebar.tsx`                           | Topic-room sidebar host; swaps to `ViewSpineTree` when `viewSpineEnabled` is ON                                                                                                                                          |
...
| `sidebar-items.ts` | Pure `buildSidebarItems()` function: merges sessions + worktrees + workspaces into `SidebarItem[]` with reconciliation. Still feeds compatibility stores and tests, but no longer drives the default sidebar UI.                                                                                                                                                                                                                |
...
- Loading state: tracked in component state; `setLoading`/`clearLoading` wrap async actions (start, kill, delete)
...
- Topic-room and action-registry controls own the current session/item affordances. Legacy repo-row context menu behavior was removed with the old sidebar fallback.
...
- **Node identity in topic-room/sidebar rows (#864)** — topic-room and tab chrome surfaces render cross-node status via shared node summary helpers; local-node sessions stay quiet, remote sessions show label + heartbeat dot (online/stale/updating/offline) where the surface includes node context.
