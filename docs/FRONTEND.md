...
- **Implemented now:** session/file `WorkspaceTab` layout, optional session `nodeId`, node-aware terminal creation and PTY routing, active-tab-derived utility rail context, guarded rail states for remote and non-git tabs, and the `shared/workspace.ts` / `shared/project.ts` / `shared/bench.ts` identity helpers (first used in production by the view-spine derive — see below).
...
- **Experimental (flag-gated):** the **view-spine MVP** (#444) — a read-only, client-derived `Workspace -> Project -> Instance -> Bench` sidebar tree behind the default-OFF `view-spine` flag. It derives over existing store data only; it is not the persisted six-layer model. See [View-Spine MVP](#view-spine-mvp-444) below.
- **Planned:** saved Views/pins, the backend source-of-truth lane (persistence/CRUD/migration, #733–#737), a Workspace bar (#728), Bench creation (#730), environment inheritance (#740), interactive tree leaves (#739), full Project/Instance/Bench CRUD, #428 remote File RPC, #470 Worker decoration, and repo-wide vocabulary/API renames.
...
The first user-facing slice of the six-layer IA: a **read-only, client-derived sidebar tree** behind a feature flag that is **OFF by default**. It renders `Workspace -> Project -> Instance -> Bench` with Tab counts as leaves; it adds **zero new persistence and zero new server endpoints**, deriving entirely from data the stores already hold. It is not the persisted six-layer model — the backend source-of-truth lane (#733–#737) is still pending, and the flag stays default-OFF until that lands.
...
- **Render swap:** `Sidebar.tsx` mounts `<ViewSpineTree>` (`frontend/src/components/ViewSpineTree.tsx`) when the flag is ON. The OFF path is byte-identical to today's repo/worktree sidebar; legacy `sidebar-items.ts` is untouched (`ViewSpineTree`/`view-tree.ts` are siblings, not replacements).
- **Derive adapter:** `frontend/src/lib/state/view-tree.ts` is a pure projection — `buildViewTree({ repos, worktrees, sessions, workspaceGroups, nodes })` joins existing store data with the shared `['hub-nodes']` TanStack query (`/nodes` status; no new fetch) and `applyLens(tree, lens)` filters it. It is the first production use of the `shared/project.ts` / `shared/bench.ts` / `shared/workspace.ts` id helpers. Layer mapping: a repo becomes a **Project** keyed on git remote identity (directory fallback keyed on node + path), so the same remote across nodes dedups to one Project with multiple **Instances** (`(Project, nodeId)`, host label `this host` for local / node `displayName` for remote, with online/stale status dots); a worktree becomes a **Bench**; sessions roll up to **Tab counts**. Sessions with no `repoPath` (free/remote) render in a separate **free / remote lane** with reduced row anatomy — repo identity and branch are structurally omitted so there is no leakage.
- **Views lenses (#727):** an ephemeral, in-memory segmented control with three lenses — **recent** (default; reorders projects/benches by rolled-up activity), **all sessions** (no filter), and **this host** (local instances only). Lens state is not persisted and resets to `recent` on reload; there are no saved Views yet.
- **"+ tab" on a git bench (#731):** a git Bench shows a `+ tab` affordance that creates an agent session anchored to the bench's `(nodeId, repoPath, worktreePath)` via the existing node-aware `createAgentSession` create path (`benchCreatePayload` resolves the payload). No environment is inherited (env inherit is deferred to #740). The affordance is withheld on non-git/directory benches, which have no `config.repos`-validated repo anchor; leaves themselves remain non-interactive counts.
- **Not shipped here (do not document as done):** persistence/CRUD/migration (#733–#737), the Workspace bar (#728), Bench creation (#730), env inheritance (#740), saved Views/pins (#738), and interactive tab leaves (#739).
...
| `Sidebar.tsx`                        | Current repo/workspace-group navigation surface; migration target is View -> Workspace -> Project -> Instance -> Bench with progressive disclosure. Swaps to `ViewSpineTree` when `viewSpineEnabled` is ON; OFF path unchanged                             |
| `ViewSpineTree.tsx`                  | Experimental, flag-gated (`view-spine`, default OFF), read-only render of the client-derived view-tree: Project/Instance/Bench rows + Tab counts, free/remote lane, Views lens selector, git-bench `+ tab`. Reuses existing sidebar primitives only (#444) |
