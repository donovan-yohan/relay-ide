...
| ------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `browser-session`         | Human browser/UI entry after PIN login and existing operator browser routes.                 | Protects the web UI from unauthenticated _browser and remote_ clients. Dev-instance flags do not satisfy this lane. It is not a fleet credential or node credential. Since #1467 it is explicitly **not** the boundary against same-OS-user processes either: a process running as the hub's uid can read the hub's config-dir CLI trust token and is trusted for local CLI use by design. |
...
### Host-local CLI trust (#1467)

Relay issue `#1467` ratifies that boundary instead of pretending otherwise. **Possession of the hub's config-directory token file — that is, filesystem access as the hub's own uid — is full local CLI trust. The PIN gates the browser/remote UI only.**

At boot the hub mints one scoped actor credential carrying the full CLI-gateway capability surface (`session:read`, `session:create:terminal`, `context:read`, `context:write`, `inbox:read`, `inbox:write`, `artifact:write`) and writes it, mode `0600`, to a port-keyed file `local-actor-token-<port>.json` in the shared standard config root (`sharedConfigRoots()`). The `relay-ide v1 …` CLI discovers that file for the loopback port it is dialing, so a fresh terminal on the hub host needs no `relay-ide login`. `relay-ide login` (#1435) is unchanged and remains the path for a CLI that is _not_ on the hub host.

Boundaries that hold:

- **No new route and no new lane.** The credential is minted in-process and delivered only through the file; nothing serves it over HTTP, logs it, or puts it in a diagnostics bundle. The failure envelope, credential records, and audit events carry ids, never token material.
- **Remote callers are unchanged.** The hub binds `0.0.0.0`, so source IP is never trusted; a caller without the file gets exactly the 401 it got before.
- **Short-lived and rotating.** The scoped actor registry is memory-only, so every hub restart rotates the credential. The on-disk credential is additionally capped at a 24-hour TTL and refreshed in place well before expiry, so a copy taken by a backup or a dotfile sync is dead within a day even on a hub that never restarts. The hub deletes the file on graceful shutdown, and only when the file still holds the credential id it published.
- **The reader fails closed.** A token file that is a symlink, group/other readable, owned by another uid, sitting in a group/world-writable directory, bound to a different port, or bound to a non-loopback hub URL is ignored, and the CLI behaves as if it were absent. The reader opens with `O_NOFOLLOW` and validates the open handle, so nothing can be swapped between the check and the read. Writes go to a uniquely named temp with `O_EXCL` and are renamed into place, so a pre-planted file or symlink is never written through.
- **Nothing is silently re-permissioned.** If the destination directory already exists and is group/world-writable, the hub refuses to publish and logs what to run (`chmod go-w …`) rather than quietly tightening a directory an operator may have shared on purpose.
- **Only a hub deployed on this machine, reachable on loopback, publishes.** The mint is skipped unless the hub's own config directory is (or sits under) a shared config root, so a fixture or test child booted with an explicit `RELAY_IDE_CONFIG` elsewhere can neither litter the operator's real config directory nor replace a live hub's port-keyed file. It is also skipped when the hub binds a non-loopback address only, since the CLI would never reach it.
- **Opt-out.** `RELAY_IDE_DISABLE_LOCAL_ACTOR_TOKEN=1` suppresses the mint entirely; e2e fixture mode (`RELAY_IDE_E2E_FIXTURES`) never mints.
- **File RPC may not read Relay's config state.** Since a session scoped to `$HOME` would otherwise put `config.json` (the PIN hash), `actor-token.json`, `node-credential.json`, the node identity key, and this token inside a legitimate read range, `server/file-rpc.ts` denies with `FILE_RPC_PROTECTED_PATH` every operation — read, list, stat, tail, write, and each poll of a `tail --follow` — whose path, realpath, cwd, or write-target parent lands inside a protected target. Protected targets are the whole of `sharedConfigRoots()` (those directories are Relay's alone), plus, for a hub pinned with `RELAY_IDE_CONFIG`, the config file itself and the credential files beside it (`actor-token.json`, `node-credential.json`, `node-identity-key.json`, `local-actor-token-*.json`). A pinned directory is _not_ denied wholesale, because `#961` explicitly invites operators to pin a config inside a repo checkout and denying that tree would break every legitimate read in it. The check is path-based: a hard link planted inside the session root still resolves to itself, so it is not caught — planting one already requires hub-uid access, which the ratified boundary trusts.
- **`config.json` is `0600`.** It carries `pinHash`; `saveConfig` now creates it owner-only and tightens an older world-readable file on the next save.

What this consciously gives up: the channel-scope narrowing on scoped actor credentials (`denyChannelReadWithoutScope` / `denyOutOfScopeChannel`) is **no longer a containment boundary against anything running on the hub host — including Relay's own agent sessions**, which run as the hub uid and can therefore read the token file and obtain the unnarrowed host-local credential regardless of the scope their own credential was issued with. Channel scoping remains meaningful against remote and delegated callers; it is not a sandbox for local processes. That is the direct consequence of the ratified boundary, not an oversight. The boot-minted credential is marked with a trusted-internal `hub-local-cli` reason that only the in-process issuer can stamp (`credentialIssueMetadata` strips it from every caller-supplied metadata blob, and renewal of such a credential is refused rather than silently downgraded); it carries operator authority for channel reads and writes, but it does **not** reach the `persistent-orchestrator` verbatim-sender-id path or the agent brake — a message posted with it is attributed as an agent sender.
...
The MVP registry lives in `shared/scoped-actor-credentials.ts` and `server/scoped-actor-credentials.ts`. It issues opaque `relay-sac-v1.<credential-id>.<secret>` bearer material, stores only a secret hash (`sha256Hex(secret)`), and exposes public credential records without token material or `secretHash`. Scoped actor credentials minted for the CLI gateway or login are persisted hash-only in SQLite (`scoped-actor-credentials.db`, mode `0600`) and restored into the in-memory registry at boot, so valid credentials within TTL survive hub restarts while revocations persist and expired rows are pruned (#1546). Registry callers must provide a bounded expiry (`expiresAt` or `ttlMs`), audience, actor, issuer, capability bits, and at least one explicit scope dimension.
