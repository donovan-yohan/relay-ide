...
- **Rotation is free.** The scoped actor registry is memory-only, so every hub restart rotates the credential and any copied or backed-up file is inert. The hub removes the file on graceful shutdown.
- **The reader fails closed.** A token file that is a symlink, group/other readable, owned by another uid, sitting in a group/world-writable directory, bound to a different port, or bound to a non-loopback hub URL is ignored, and the CLI behaves as if it were absent. Writes go to a uniquely named temp with `O_EXCL` and are renamed into place, so a pre-planted file or symlink is never written through.
...
- **File RPC may not read the config roots.** Since a session scoped to `$HOME` would otherwise put `config.json` (the PIN hash), `actor-token.json`, `node-credential.json`, the node identity key, and this token inside a legitimate read range, `server/file-rpc.ts` denies every operation whose path (or its realpath) lands inside `sharedConfigRoots()` with `FILE_RPC_PROTECTED_PATH`.
