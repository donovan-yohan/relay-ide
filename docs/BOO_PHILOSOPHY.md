...
Relay should feel like a channel-first collaboration product with a scriptable
terminal multiplexer and operator cockpit underneath it. Agents participate in
channels and DMs; public `Session` records are human-driven terminals, not
standalone agent conversations.

The durable rules are:

1. **CLI/API first.** Anything important in the web UI needs a stable `relay-ide v1 ... --json` command or documented API. Agents should never need browser scraping, private `/hub/node-link` messages, provider profile DB reads, or backend-private terminal commands when Relay can own the primitive.
2. **Session identity is terminal identity.** A terminal pane, browser tab,
   mobile card, command-palette result, or dashboard row is a view over a
   human-driven terminal `Session` inside a `WorkContext`. Agent participant
   identity belongs to a channel profile actor.
...
5. **Capabilities and audit wrap terminal control.** Sends, keys, attach, kill,
   artifact reads, file operations, and future screen APIs must stay inside
   Relay's `CapabilityGrant`, `AuditEvent`, and scoped credential model.

The retired terminal-agent model is not a compatibility target. Public
sessions do not launch provider CLIs as agent participants, do not become
agent-driven, and do not support a hand-back transition. Supervisor commands
operate a human-driven terminal only. Handoffs may produce a plan and expose
bounded artifact evidence, but they do not launch a destination agent or
orchestrate a second conversation. Workflow runs are generic, redacted
...
- Stable human-driven terminal descriptor/lifecycle commands:
  `sessions.list`, `sessions.get`, `sessions.create`, `sessions.attach`,
  `sessions.detach`, `sessions.kill`, and `sessions.rename`.
...
- Channel and DM conversations as the only agent-participation surface;
  terminal sessions are not agent identities or provider runtimes.
...
- Say **human-driven terminal session** for public `Session` records. Never use
  public session ids as agent participant ids.
...
- Do not document terminal-agent launch, provider resume, agent-driven/co-driven
  control, hand-back, or workflow planner/worker topology as supported.
- Describe `handoffs.plan` as a read-only plan and `artifacts.read` as a passive,
  bounded evidence read. A plan or artifact reference never launches work.
- Do not tell adapters to use internal REST routes, browser WebSockets,
  node-link, backend-private terminal commands, or private provider stores as
  contracts.
