...
- L-20260801-derived-topics-are-all-or-nothing: `GET /workspace-topics` computes `derived = persisted.length === 0` and then returns `fallbackTopics(...)` OR `persisted` — never the union (`server/workspace-topics.ts`, the list route). A single persisted row therefore hides EVERY WorkContext-derived row, and the `derived` flag the client branches on flips to `false` in the same step. That is intentional, not a bug: the derived lane is a cold-start scaffold for a hub that has no channels yet, and unioning it with real rows would resurrect archived or deleted channels as permanent ghosts the operator cannot dismiss. Scope filtering survives both branches — `fallbackTopics` takes the same `workspaceId`, so a derived list is still workspace-scoped. The SEARCH path deliberately does not share the rule: `collectSearchTopics` always unions persisted + derived and reports `derived` only as "nothing persisted matched", so search can still reach a WorkContext that a store row masks in the list.
