...
- L-20260729-client-derived-unread: unread state is client-derived: persisted `lastRead` (localStorage) vs live `latestSeq` (in-memory). Any in-memory half must be seeded from a list payload on mount or every page reload renders fully read. Seeding races with mark-as-read: guard stale refetch payloads with a per-channel clamp-epoch fence compared against the query's `dataUpdatedAt` (PR #1290). Extended by #1308 slice 3, which adds a DURABLE hub copy of the operator's own marks without moving the verdict off the client: the hub is a point of CONVERGENCE, not a source of truth, so localStorage stays the fast path and an unreachable hub costs sync and nothing else. Three rules make that safe, and all three are about direction. (1) Every hub-sourced mark — boot seed AND live `channel-read-state` broadcast — merges MONOTONIC-UP through ONE store action (`mergeReadState`), because raising a marker can only clear unread: a phone that woke up on seq 12 can never turn 388 messages unread again on a desktop that read to 400. Two devices trading push-backs converge rather than ring, since values strictly increase and a matching mark schedules nothing. (2) The MAX is against `max(store, localStorage)`, never the store alone — on a cold boot `lastReadByChannel` is empty while the real marker sits in localStorage, so a store-only comparison lets a lower hub mark win and relights every already-read channel. Any store that mirrors a persisted value has this trap the instant a second writer appears. (3) The #1178 clamp is the one case where the correct move is DOWN, and monotonic-up cannot express it — so hub payloads run through the SAME `clampedAt >= fetchedAt` fence as channel-list payloads (one epoch, both merges), and a fenced-out row is skipped in BOTH directions: no merge and no push-back, or the fence would leak the retracted position straight back to the hub. Corollary on the push side: a read mark is worth exactly one best-effort request. No retry timer — the next mark supersedes it and the next boot's merge re-pushes anything the hub is behind on — but a trailing (non-resetting) per-channel window plus a `pagehide` + hidden-`visibilitychange` flush is mandatory, and the flush must use `fetch(..., {keepalive:true})`: an ordinary fetch is cancelled on unload, and `sendBeacon` cannot carry the `x-relay-capabilities` header Relay routes require. Two amendments from the #1308 slice 3 review, both about things a teardown flush cannot see. (a) A teardown flush LOSES to the mark it exists to publish whenever the writer is a later `visibilitychange` listener: DOM listeners fire in registration order, the store binds lazily on the first armed push, and ChannelView re-registers per channelId — so the flush runs first and finds nothing armed, and deferring does not help (a microtask checkpoint runs after each listener, still ahead of the next one). Publish INLINE when `document.hidden` is already true; ordering-independence is the only fix. (b) A best-effort push still needs to read its own response, and the DIRECTION of that response decides which lane it belongs to. At or above the pushed mark it goes through the SAME monotonic-up merge — stamped with the time the PUT was ISSUED, so an in-flight clamp still fences it — which picks up another device's higher mark without waiting for the broadcast. BELOW the pushed mark it is not an opinion at all: `Math.min(requested, head)` is the only branch of the store write that can return less than the request, so the answer is provably the channel HEAD, and the right response is the #1178 repair rather than a memo about a refusal (#1318). Drive `clampChannelStores` with it: the stranded marker is rewritten to head, the unread dot it was silently eating comes back with the next message, and the per-boot re-push loop closes at the source because the local mark now equals the hub's. The suppression table this replaced was the wrong shape twice over — it also silenced every later mark BELOW the stranded value (a whole stretch of the recreated channel's life), and being a suppression lane it needed its own clamp-epoch fence. The clamp's two halves then need DIFFERENT fences, and "it only moves DOWN" is an argument about the wrong competitor: it disposes of a competing CLAMP (a deeper local one that overtook the PUT is simply a no-op against stores already below the head) but says nothing about a competing RAISE, which is exactly the direction the clamp destroys. So fence the HEAD half — a `channel-activity` broadcast that landed while the PUT was open is newer than the head in the body, and lowering `latestSeqByChannel` back to that snapshot deletes the message the repair exists to surface — by stamping when each channel's head was last raised (`activityRaisedAtByChannel`, `Date.now()` on broadcast, the payload's `fetchedAt` on seed) and skipping the half when the stamp is at or after the caller's `observedAt`. Leave the READ half unfenced: DOWN on a read marker only ADDS unread, so a stale head costs one spurious dot, and the only local raise it could lose is one ABOVE the stranded marker, which the hub's answer proves does not exist. Corollary on scope: a helper written for a caller whose argument is authoritative-at-apply-time (the channel socket's own full snapshot precedes every later message on that socket) grows a fence the moment a caller with a network-delayed snapshot reuses it — the reuse is right, the unconditional trust is not. Second-order effect worth stating out loud rather than burying: retiring the marker to head declares 1..head READ, so the dot returns with the NEXT message and any attention badge armed at or below head is cleared. General rule: when a server's refusal is derivable authoritative state, apply it as state, do not remember it as a veto — but apply it with the freshness fence any other stale-snapshot write would need. Related: a persisted marker parsed with bare `Number()` yields NaN on corruption, and NaN makes both merge branches false — coerce non-finite to 0 or the channel silently leaves the sync in both directions.
...
- L-20260801-derived-topics-are-all-or-nothing: `GET /workspace-topics` computes `derived = persisted.length === 0` and then returns `fallbackTopics(...)` OR `persisted` — never the union (`server/workspace-topics.ts`, the list route). A single UNARCHIVED persisted row therefore hides EVERY WorkContext-derived row, and the `derived` flag the client branches on flips to `false` in the same step. That is intentional, not a bug: the derived lane is a cold-start scaffold for a hub that has no channels yet, and unioning it with real rows would resurrect archived or deleted channels as permanent ghosts the operator cannot dismiss. Scope filtering survives both branches — `fallbackTopics` takes the same `workspaceId`, so a derived list is still workspace-scoped. The SEARCH path deliberately does not share the rule: `collectSearchTopics` always unions persisted + derived and reports `derived` only as "nothing persisted matched", so search can still reach a WorkContext that a store row masks in the list. Corollary — the guard is CONDITIONAL, and the word "unarchived" is load-bearing: `persisted` is fetched with `includeArchived`, and the store filters `status != 'archived'` when that is false, so with every channel in a workspace archived `persisted.length === 0` and the list flips BACK to the derived scaffold, re-surfacing exactly the archived WorkContexts the rule exists to suppress (derived rows are keyed `derived-<contextId>`, so they neither dedupe against the archived rows nor inherit the archive filter). Accepted debt as of #1287 slice 0, recorded rather than fixed because "what does an all-archived workspace render" is a product call; gating the branch on the UNFILTERED count (`includeArchived: true`) closes it. General shape: when an all-or-nothing fallback keys off a FILTERED count, state the filter in the rule — "no rows" and "no rows you can currently see" are different predicates and the empty-after-filtering case is the one that resurrects the ghosts.
...
- L-20260801-latched-surface-needs-a-shared-exit: `resolveAppViewMode` is a PRIORITY ladder, so any boolean that sits high in it (`activeChannelId`, then `topicComposerOpen`) is a LATCH: a navigation that sets its own destination without clearing the higher latch is a silent no-op — the screen never changes and no request is issued — and the flag it left behind fires as a surprise navigation the moment the operator closes the channel. This shipped twice in one epic (slice 1's cockpit hatches, then the new-chat button in #1287) because the rule lived only in prose and was hand-implemented per call site, and because the component tests for those sites stubbed the navigation itself with `vi.fn()` — a stub of the broken function can never fail. Two fixes, both required: ONE named exit (`leaveChatSurface()`) that every non-chat navigation routes through, and a table-driven test (`test/chat-surface-navigation.test.ts`) that enumerates the entry points, seeds every latch, and asserts the surface each action's NAME promises. Corollary: prefer wiring the real navigation in component tests; a `vi.fn()` on the exact function under suspicion is a green light with nothing behind it.
- L-20260801-lane-and-repo-are-one-decision: `activeWorkspaceId` (which lane a chat is FILED in) and `activeRepoPath` (what it is ROUTED at) are two halves of one routing decision that `useTopicRoomCreate` reads separately. Fixing only one half turns a consistent-but-stale create into a create SPLIT across two projects, which is worse. When a fix moves one pointer of a derived tuple, enumerate every other pointer the same consumer reads. The repo half is stamped on the CREATE paths only, never on a bare lane click: `activeRepoPath` sits directly above the chat landing in the same priority ladder, so writing it outside a create would silently convert selecting a lane into a navigation onto RepoDashboard.
- L-20260801-composer-has-no-url: the topic composer is a store flag with no route, so any navigation that opens it while clearing a routed field (the #1287 new-chat fix clears `activeChannelId`) makes `useUrlNav`'s push effect write a history entry for the surface UNDERNEATH it (`/<repoHash>/<sessionId>` or `/`), which re-renders as the terminal/RepoDashboard on reload. Accepted as of #1287: Back correctly returns to the channel, which is a net improvement over pushing nothing, and the identical state was already reachable by opening the composer over a session. Closing it means giving the composer a route (`/new`). General shape: the moment an unrouted surface starts clearing routed state, it has become a navigation and owes the URL a representation.

---

## Sep 2026 — orchestrator wake signal for channel runs (#1570)

- `channels.wait` is the **final-response lane**: it returns only the final assistant principal text item for the correlated run (no thoughts, tool cards, or progress rows), plus contract summary/exit codes for automation.
- `channels history --run ...` is the **inspect lane**: it returns ordered durable items for the run (including tool/system rows) for audit/debugging.
- For subscriptions, persist/resume with `durableSeq` (not transient sequence ids), and use `channels.subscribe --only run-terminal,system` when you want a bounded wake-signal projection without losing resume semantics.
