...
| `handleArchive` | `useSessionHandlers.ts` -> `killSession()` then `deleteWorktree()` | none | `needs gateway command` | `kani-backend` | Destructive session + worktree cleanup needs a typed confirmation/audit path before agent exposure. |
...
| `workspace.new-worktree` | `useActionRegistry.ts` -> `handleNewWorktree()` | partial: `sessions.create` after private worktree create | `needs gateway command` | `kani-backend` | Needs a typed worktree-create path before agent parity. |
...
| UI action id / entrypoint | Current handler path | Existing CLI/API command | Gap classification | Recommended owner | Notes |
| --- | --- | --- | --- | --- | --- |
| `pr.create` | `useActionRegistry.ts` -> `navigateToDashboard()` | none for dashboard navigation | `ui-only non-agent surface` | `ika-frontend` | Registered palette shortcut only opens the dashboard. A future PR-create mutation command must be defined as a separate verified workflow before agent exposure. |
| `pr.push-branch` | `useActionRegistry.ts` -> `navigateToDashboard()` | none for dashboard navigation | `ui-only non-agent surface` | `ika-frontend` | Registered palette shortcut only opens the dashboard; do not count it as branch-push parity gap without a real mutation handler. |
| `pr.switch-branch` | `useActionRegistry.ts` -> `navigateToDashboard()` | none for dashboard navigation | `ui-only non-agent surface` | `ika-frontend` | Registered palette shortcut only opens the dashboard. Branch checkout/worktree mutation needs a separate command/workflow if Relay owns it. |
| `pr.fix-conflicts` | `useActionRegistry.ts` -> `navigateToDashboard()`; direct handler exists as `handleFixConflicts()` | none for dashboard navigation; direct flow partially maps to `sessions.create` + input/supervisor commands | `ui-only non-agent surface` | `ika-frontend` | Registered palette shortcut only opens the dashboard. The real conflict-fix workflow is tracked separately as `handleFixConflicts()`. |
| `pr.archive-branch` | `useActionRegistry.ts` -> `navigateToDashboard()` | none for dashboard navigation | `ui-only non-agent surface` | `ika-frontend` | Registered palette shortcut only opens the dashboard. Destructive branch/worktree archive semantics need a separate audited command if exposed. |
| `pr.rename-branch` | `useActionRegistry.ts` -> `navigateToDashboard()` | none for dashboard navigation | `ui-only non-agent surface` | `ika-frontend` | Registered palette shortcut only opens the dashboard. Branch rename mutation needs a separate stable command if Relay owns it. |
...
| `pr.change-target` | `useActionRegistry.ts` -> `navigateToDashboard()` | none for dashboard navigation | `ui-only non-agent surface` | `ika-frontend` | Registered palette shortcut only opens the dashboard. Target-branch mutation needs a separate stable command if Relay owns it. |
| `pr.skip-checks` | `useActionRegistry.ts` -> `navigateToDashboard()` | none for dashboard navigation | `ui-only non-agent surface` | `ika-frontend` | Registered palette shortcut only opens the dashboard. Release/check policy overrides need a separate explicit command/audit path if they become agent-operable. |
| `dashboard.open-pr-session` | `useActionRegistry.ts` -> `handleQuickAgent()`; direct dashboard `handleOpenPrSession()` opens PR branch | partial: `sessions.create` | `needs gateway command` | `kani-backend` | Registered action is too generic; the real PR branch workflow needs a typed branch/worktree selection path. |
...
| `settings.connect-github` | `useActionRegistry.ts` -> settings integrations section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. A real GitHub connect/auth mutation should be inventoried as a separate verified entrypoint/command. |
| `settings.disconnect-github` | `useActionRegistry.ts` -> settings integrations section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. A real disconnect mutation should be a separate audited command if agent-operable. |
| `settings.setup-webhooks` | `useActionRegistry.ts` -> settings integrations section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. A real webhook setup mutation should be inventoried separately before agent exposure. |
| `settings.remove-webhook` | `useActionRegistry.ts` -> settings integrations section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. A destructive webhook removal command would need separate typed confirmation/audit. |
| `settings.test-webhook` | `useActionRegistry.ts` -> settings integrations section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. Agent-operable webhook tests should be separate verified commands. |
| `settings.connect-jira` | `useActionRegistry.ts` -> settings integrations section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. A real Jira connect/auth mutation should be inventoried separately. |
| `settings.disconnect-jira` | `useActionRegistry.ts` -> settings integrations section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. A real disconnect mutation should be a separate audited command if agent-operable. |
...
| `settings.clear-analytics` | `useActionRegistry.ts` -> settings advanced section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. Any real analytics deletion command should be inventoried separately with destructive confirmation/audit. |
| `settings.toggle-continue` | `useActionRegistry.ts` -> settings general section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. A persisted continue-default mutation should be a separate verified command. |
| `settings.toggle-notifications` | `useActionRegistry.ts` -> settings general section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. A persisted notification-default mutation should be a separate verified command. |
| `settings.change-default-agent` | `useActionRegistry.ts` -> settings general section | none for dialog navigation | `ui-only non-agent surface` | `ika-frontend` | Settings-section opener only. A default-agent mutation should be a separate verified command. |
