...
Forty-two TypeScript modules (plus `adapters/`, `output-parsers/`, and `protocol-adapters/` subdirectories) compiled to `dist/server/` via `tsc`. Modules communicate via ESM `import` statements.
...
| `shared/mobile-input-pipeline.ts`       | Pure-function event-intent pipeline for mobile virtual keyboard input — shared by the React `MobileInput` component and server-side tests; unit-tested via JSON fixtures                                                                                           |
...
| `shared/chat-events.ts`                 | Shared `ChatEvent` discriminated union and type guards — imported by both server and frontend as the wire protocol for web-chat transcripts                                                                                                                        |
...
| ADR     | Topic                                                                              |
| ------- | ---------------------------------------------------------------------------------- |
| ADR-001 | Modular server architecture (forty-two modules, composition root, dependency flow) |
