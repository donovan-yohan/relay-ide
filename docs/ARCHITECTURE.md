...
| `auth.ts`                                              | PIN hashing (scrypt), rate limiting (5 fails = 15-min lockout), browser-session cookies                                                                                                                                                                            |
...
| `work-context-messages.ts`                             | SQLite-backed append-only WorkContext message store with bounded list/query indexes for WorkContext, thread, parent, payload schema, sender, and task/context refs                                                                                                 |
| `features/work-context-message-router.ts`              | CLI/API-facing WorkContext message router: append/list/show/query, scoped actor WorkContext enforcement, redacted metadata events, and fail-closed unavailable-store responses                                                                                     |
| `workflow-runs.ts` / `features/workflow-run-router.ts` | SQLite-backed workflow run projections: provider-runtime workflow metadata plus Relay orchestration topology for visible planner/worker sessions, all scoped to WorkContext and redacted metadata events                                                           |
...
| `scoped-actor-credentials.ts`  | Scoped actor credential registry primitive: issue/validate/revoke/expiry lifecycle, bounded actor/audience/capability/scope checks, token-hash storage, and redacted audit helper generation                      |
| `work-context-messages.ts`     | WorkContext message envelope contract: opaque repo/agent-defined payloads, refs/threading metadata, redaction normalization, and append/list/show/query input/output parsing                                      |
| `workflow-run.ts`              | Workflow run contract: bounded provider-runtime projections plus optional Relay orchestration topology linking planner and worker sessions without raw transcripts/provider private state                         |
| `orchestration-run-launch.ts`  | Pure orchestration launcher coordinator: validates trusted launch payloads, composes workflow-run/session/inbox dependencies, records child topology, and reports partial lane failures without provider glue     |
| `workspace-evidence.ts`        | Workspace evidence contract: capability-tagged roots (workspace/project/bench/repo/worktree/directory), bounded list/stat/read/preview shapes, availability/preview state machines, and byte/entry caps           |
...
| `frontend/src/lib/state/`             | Pure logic modules (`display-state.ts` — 6-state display state machine, `sidebar-items.ts` — legacy SidebarItem reconciliation for session/unread state, `attention.ts` — state priority scoring) |
...
```
Browser (xterm.js) <--WebSocket /ws/:id--> ws.ts <--session backend abstraction--> relay-pty direct PTY/libghostty-vt
                                              |
...
Node    <--terminal backend: relay-pty/libghostty-vt--> shell / agent CLI
```
...
xterm.js remains the browser renderer. Interactive agent and terminal sessions use the direct `relay-pty` backend, backed by node-pty plus the libghostty-vt terminal model. `tmux-compat` is no longer a supported runtime backend; saved tmux-backed metadata is treated as legacy/tombstoned state, not restored.

`relay-pty` is not a process supervisor. Browser reconnects, browser refreshes, and hub/node link reconnects can reattach to a live Relay process and replay bounded scrollback. If the Relay server process restarts, resume is cold: Relay reads saved session metadata and scrollback, rebuilds session rows, and starts a new child PTY/session if appropriate. Preserving a live child process across server restart requires future daemon/supervisor work.

`terminalBackend` remains a compatibility field, but only `relay-pty` is accepted. `tmux-compat` and old `useTmux`/`tmuxSessionName` serialized fields must fail closed or be ignored as unsupported legacy state.

Browser-level tabs and panes can be rearranged freely without becoming the process boundary. `sessions.ts` exposes backend-neutral terminal helpers (`sendTerminalKeys`, `sendTerminalText`, `captureTerminalVisibleText`) for terminal input and rendered/visible screen capture over the `relay-pty` model.
...
**Auth:** HTTP and WebSocket routes are grouped by auth lane in `server/auth.ts` (`AUTH_ROUTE_LANE_INVENTORY`). Browser UI routes use browser-session cookies from PIN login or first-run PIN setup; scoped actor credentials are delegated bounded capabilities for agents, CLIs, and automation systems; CLI gateway routes currently accept browser-session compatibility while naming `scoped-actor-credential` as the migration lane; node heartbeat and `/hub/node-link` use node credentials; pairing exchange uses pair tokens; setup/login/health routes are public-local-only. Dev-instance flags isolate config/ports/process behavior and do not satisfy auth. Rate limiting is per-IP for browser PIN attempts.

**Session lifecycle:** Runtime session records are in-memory during normal operation. `relay-pty` owns direct PTY processes while the Relay server process is alive. Multiple sessions per directory are allowed (multi-tab support). PTY exit triggers automatic cleanup. Scrollback buffers cap at 256KB with FIFO eviction. PTY spawns are wrapped with `trap '' PIPE; exec` to prevent SIGPIPE from killing sessions. During auto-updates, sessions are serialized to disk (`pending-sessions.json` + scrollback files) and restored on restart as cold resume from metadata/scrollback; Relay does not preserve the live child process across server restart without future daemon/supervisor work.
