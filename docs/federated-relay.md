...
This corrects the older shorthand from #427-era docs where "auth cookie" could read like global Relay authorization. #427 remains the predecessor for trust tiers, capability policy, audit, confirmation, and credential rotation. #797 is the broader auth-model epic, #798 split route inventory and browser-session wording, and #802 adds scoped actor token issuance/validation/revocation/expiry. #802 does not add node proof-of-possession, passkeys, TOTP, a new human approval flow, or complete CLI gateway command migration.
...
- **Hermes/GitHub/Kanban/native-agent replacement** — Relay links and controls existing systems through scoped refs and adapters; it does not clone their dashboards or become their storage/runtime owner. Relay stays the conversation and live-control layer. The one verified mapping in this direction is the agent-profile `hermesProfile` binding: a Relay hermes profile addresses a named Hermes multiplex profile at `/p/<profile>`, so a mention runs under that profile's own Hermes-side config and state. Verify it with the `/p/<id>/v1/models` matrix in `docs/references/hermes-multiplex-setup.md`, not by assumption.
