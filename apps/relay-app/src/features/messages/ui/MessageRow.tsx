...
import { AgentRunPill } from "@/features/agent-runs/ui/AgentRunPill";
...
export const MessageRow = React.memo(
  function MessageRow({
    ...
    const runId = message.tags?.find((t) => t[0] === "run_id")?.[1];
    const runDuration = message.tags?.find((t) => t[0] === "run_duration")?.[1];
    const toolCallCount = Number(message.tags?.find((t) => t[0] === "tool_call_count")?.[1] || 0);
    const filesTouchedCount = Number(message.tags?.find((t) => t[0] === "files_touched_count")?.[1] || 0);
    const pendingApproval = message.tags?.find((t) => t[0] === "pending_approval")?.[1] === "1";

    const isAgentAuthor = Boolean(isAuthorAgent || message.isAgent);

    const handleOpenRun = React.useCallback(() => {
      if (message.pubkey) {
        onOpenAgentSession?.(message.pubkey, channelId, runId ?? null);
      }
    }, [message.pubkey, channelId, runId, onOpenAgentSession]);

    const runPillNode = isAgentAuthor && (runId || toolCallCount > 0 || runDuration || pendingApproval) ? (
      <AgentRunPill
        runId={runId}
        durationLabel={runDuration}
    ...
    const avatarGutterNode = isDisplayedAsContinuation ? (
      ...
      <button
        className={cn(
          "flex shrink-0 items-start cursor-pointer focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring",
          avatarButtonRadiusClass,
        )}
        type="button"
        onClick={handleOpenRun}
        title={`View ${message.author} run process`}
      >
        {avatarNode}
      </button>
    ) : message.pubkey ? (
    ...
    const continuationMetadataNode =
      isDisplayedAsContinuation && (statusMetadataNode || runPillNode) ? (
