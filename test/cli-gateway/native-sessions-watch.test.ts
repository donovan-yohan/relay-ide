...
async function makeSessionFile(
  ...
  const root = await mkdtemp(path.join(tmpdir(), `relay-live-tail-${providerPrefix}-`));
...
describe('JsonlFileTailer', () => {
  ...
  it('handles truncation/rotation by resetting to the start of the new file', async () => {
    ...
    const tailer = new JsonlFileTailer<Record<string, unknown>>({ filePath: file });
    ...
    expect(poll.offset).toBeLessThanOrEqual((await import('node:fs')).statSync(file).size);
