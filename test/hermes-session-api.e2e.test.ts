...
function writeWorkingHermesStub(tmpDir: string): void {
  ...
  // The adapter passes API_SERVER_PORT in the environment; the default CLI args
  // include Hermes profile flags before `gateway run`.
  writeAgentStub(
    tmpDir,
    'hermes',
    `#!/usr/bin/env node
...
const port = process.env.API_SERVER_PORT || (portIdx !== -1 ? args[portIdx + 1] : process.argv[2]);
...
test('POST /sessions returns structured json when hermes gateway startup fails', async () => {
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'relay-hermes-api-e2e-')
  );
  writeAgentStub(
...
`
  ...
  try {
    ...
    await expect(createRes.json()).resolves.toMatchObject({
      ...
      message:
        'Hermes gateway exited before API server became ready (code 2): mock hermes gateway failed to start',
    });
  } finally {
    await stopRelayServer(server);
  }
