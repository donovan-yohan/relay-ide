...
test.describe('ShortcutHint Component', () => {
  ...
  test('renders shortcut hint for action with shortcut', async ({ page }) => {
    const hint = page
      .locator('.hint-row')
      .filter({ hasText: 'With shortcut (mod+shift+n)' })
  ...
  test('renders global shortcut', async ({ page }) => {
    const hint = page
      .locator('.hint-row')
      .filter({ hasText: 'With global shortcut (mod+k)' })
      .locator('.shortcut-hint');
    await expect(hint).toBeVisible();
  ...
  test('does not render for action without shortcut', async ({ page }) => {
    const row = page
      .locator('.hint-row')
      .filter({ hasText: 'Without shortcut:' });
  ...
  test('does not render for non-existent action', async ({ page }) => {
    const row = page
      .locator('.hint-row')
      .filter({ hasText: 'Non-existent action:' });
    const hint = row.locator('.shortcut-hint');
    await expect(hint).not.toBeVisible();
  });
  ...
  test('applies custom className', async ({ page }) => {
    const hint = page
      .locator('.hint-row')
      .filter({ hasText: 'With custom className' })
      .locator('.shortcut-hint');
