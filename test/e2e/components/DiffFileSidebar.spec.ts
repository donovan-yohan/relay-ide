...
test.describe('DiffFileSidebar React component', () => {
  ...
  test('renders file list with status icons and diff stats', async ({
    page,
  }) => {
  ...
  test('clicking a file makes it active', async ({ page }) => {
    const files = page
      .locator('.diff-sidebar')
      .first()
      .locator('.sidebar-file');
  ...
  test('screenshot matches baseline', async ({ page }) => {
    await expect(page).toHaveScreenshot('diff-file-sidebar.png', {
      fullPage: true,
    });
  });
});
