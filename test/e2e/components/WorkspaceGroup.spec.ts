...
test.describe('WorkspaceGroup React component', () => {
  ...
  test('shows group name', async ({ page }) => {
    await expect(page.locator('.group-name').first()).toContainText(
      'my-workspace'
    );
  });
  ...
  test('shows launching state with spinner', async ({ page }) => {
    ...
    await expect(
      launchingGroup.locator('.launch-row').locator('text=launching...')
    ).toBeVisible();
  });

  test('screenshot matches baseline', async ({ page }) => {
    await expect(page).toHaveScreenshot('workspace-group.png', {
      fullPage: true,
    });
  });
});
