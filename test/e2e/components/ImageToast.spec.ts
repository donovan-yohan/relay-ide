...
test.describe('ImageToast React component', () => {
  ...
  test('shows insert action when requested', async ({ page }) => {
    ...
    await expect(page.locator('.image-toast-text')).toHaveText(
      'Image ready to insert'
    );
    await expect(
      page.locator('.image-toast button', { hasText: 'Insert' })
    ).toBeVisible();
  });
  ...
  test('auto-dismisses when there is no pending image path', async ({
    page,
  }) => {
  ...
  test('does not auto-dismiss while an image path is pending', async ({
    page,
  }) => {
    ...
    await expect(page.locator('.image-toast')).toBeVisible();
    await expect(page.locator('.image-toast-text')).toHaveText(
      'Keeps open while image path exists'
    );
  });
});
