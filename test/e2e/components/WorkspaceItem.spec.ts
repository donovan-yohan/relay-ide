...
test.describe('WorkspaceItem React component', () => {
  ...
  test('shows session count badge for multi-session groups', async ({
    page,
  }) => {
  ...
  test('shows add worktree button', async ({ page }) => {
    ...
    await expect(page.locator('.add-worktree-btn').first()).toContainText(
      '+ new worktree'
    );
  });
  ...
  test('shows loading state for resuming worktree', async ({ page }) => {
    ...
    await expect(loadingRow.locator('.session-name')).toContainText(
      'resuming...'
    );
  });

  test('screenshot matches baseline', async ({ page }) => {
    await expect(page).toHaveScreenshot('workspace-item.png', {
      fullPage: true,
    });
  });
});
