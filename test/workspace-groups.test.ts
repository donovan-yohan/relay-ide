...
test('workspace session creates only a terminal in the primary repo', async () => {
  const repoPath = path.join(tmpDir, 'repo-terminal');
  fs.mkdirSync(repoPath, { recursive: true });
  writeConfig({
    configVersion: 4,
    repos: [repoPath],
  ...
  expect(createParams.agent).toBeUndefined();
  expect(createParams.claudeArgs).toBeUndefined();
  expect(createParams.yolo).toBeUndefined();
});
...
test.each([
...
])('workspace session rejects removed agent fields: %j', async (payload) => {
  const repoPath = path.join(tmpDir, 'repo-agent-reject');
  fs.mkdirSync(repoPath, { recursive: true });
  writeConfig({
    configVersion: 4,
    repos: [repoPath],
  ...
  const result = await req('POST', '/workspace-groups/ws-1/session', payload);
  expect(result.status).toBe(400);
