...
test.describe('SessionStatusBar React component', () => {
  ...
  test('renders telemetry and activity for active session', async ({ page }) => {
    ...
    await expect(bar.locator('.status-rate-limits')).toHaveText('5h: 42% | 7d: 63%');
  });

  test('applies warning and danger context classes', async ({ page }) => {
    const warningBar = page.locator('#warning-container .session-status-bar').first();
    await expect(warningBar.locator('.status-context')).toHaveClass(/status-context--danger/);
  });

  test('renders placeholders when telemetry is missing', async ({ page }) => {
    const missingBar = page.locator('#missing-container .session-status-bar').first();
