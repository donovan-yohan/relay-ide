...
test.describe('FilterChipBar React component', () => {
  ...
  test('marks active chips and toggles them', async ({ page }) => {
    const activeChip = page.locator('#interactive-bar-container .filter-chip', { hasText: 'Open' });
    await expect(activeChip).toHaveClass(/active/);

    await page.locator('#interactive-bar-container .filter-chip', { hasText: 'Review' }).click();
    await expect(page.locator('#active-state')).toHaveText(/open, review/);
    await expect(page.locator('#interactive-bar-container .filter-chip', { hasText: 'Review' })).toHaveClass(/active/);
  });

  test('renders search input and clear button for active filters', async ({ page }) => {
  ...
  test('screenshot - interactive state', async ({ page }) => {
    ...
    await expect(container).toHaveScreenshot('filter-chip-bar-interactive.png', {
      maxDiffPixels: 120,
      threshold: 0.2,
    });
  });
