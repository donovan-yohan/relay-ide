...
test.describe('SessionIndicator React component', () => {
  ...
  test('renders correct character for each state', async ({ page }) => {
    ...
    for (const [state, expectedChar] of Object.entries(stateChars)) {
      const indicator = container.locator(`.status-item:has(.status-label:text-is("${state}")) .session-indicator`);
  ...
  test('applies pulse-fast for permission and needs-answer states', async ({ page }) => {
    const container = page.locator('#all-states-container');
    
    const permissionIndicator = container.locator(`.status-item:has(.status-label:text("permission")) .session-indicator`);
    await expect(permissionIndicator).toHaveClass(/pulse-fast/);

    const needsAnswerIndicator = container.locator(`.status-item:has(.status-label:text("needs-answer")) .session-indicator`);
    await expect(needsAnswerIndicator).toHaveClass(/pulse-fast/);
  });

  test('applies pulse-slow for unseen-idle state', async ({ page }) => {
    const container = page.locator('#all-states-container');
    const unseenIdleIndicator = container.locator(`.status-item:has(.status-label:text("unseen-idle")) .session-indicator`);
    await expect(unseenIdleIndicator).toHaveClass(/pulse-slow/);
  });

  test('does not apply pulse class for non-pulsing states', async ({ page }) => {
    const container = page.locator('#non-pulse-container');
    const runningIndicator = container.locator(`.status-item:has(.status-label:text("running")) .session-indicator`);
    const className = await runningIndicator.getAttribute('class');
    expect(className).not.toMatch(/pulse-fast|pulse-slow/);
  });

  test('applies bold style for permission and needs-answer states', async ({ page }) => {
  ...
  test('has correct aria-label for permission state', async ({ page }) => {
    const container = page.locator('#all-states-container');
    const permissionIndicator = container.locator(`.status-item:has(.status-label:text("permission")) .session-indicator`);
    await expect(permissionIndicator).toHaveAttribute('aria-label', 'needs approval');
  });

  test('has correct aria-label for needs-answer state', async ({ page }) => {
    const container = page.locator('#all-states-container');
    const needsAnswerIndicator = container.locator(`.status-item:has(.status-label:text("needs-answer")) .session-indicator`);
    await expect(needsAnswerIndicator).toHaveAttribute('aria-label', 'needs answer');
  });

  test('has correct aria-label for unseen-idle state', async ({ page }) => {
    const container = page.locator('#all-states-container');
    const unseenIdleIndicator = container.locator(`.status-item:has(.status-label:text("unseen-idle")) .session-indicator`);
    await expect(unseenIdleIndicator).toHaveAttribute('aria-label', 'idle, unread');
  });

  test('has correct aria-label for seen-idle state', async ({ page }) => {
    const container = page.locator('#all-states-container');
    const seenIdleIndicator = container.locator(`.status-item:has(.status-label:text-is("seen-idle")) .session-indicator`);
  ...
  test('screenshot - all session states', async ({ page }) => {
    ...
    await expect(container).toHaveScreenshot('session-indicator-all-states.png', {
      maxDiffPixels: 100,
      threshold: 0.2,
    });
  });
