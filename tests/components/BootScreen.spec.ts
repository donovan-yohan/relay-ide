...
test.describe('BootScreen React component', () => {
  ...
  test('renders loading state with app name and status text', async ({ page }) => {
    const screen = page.locator('#boot-loading .boot-screen');
    await expect(screen).toBeVisible();
    await expect(screen.locator('.boot-screen__logo')).toHaveText('claude-remote');
    await expect(screen.locator('.boot-screen__status')).toHaveText('Connecting to server...');
  });
  ...
  test('renders error state with error message', async ({ page }) => {
    ...
    await expect(screen).toBeVisible();
    await expect(screen.locator('.boot-screen__error')).toContainText('Unable to connect to the server.');
