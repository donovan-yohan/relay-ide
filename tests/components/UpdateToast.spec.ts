...
test.describe('UpdateToast React component', () => {
  ...
  test('shows the update toast when an update is available', async ({ page }) => {
    const toast = page.locator('.update-toast');
    await expect(toast).toBeVisible();
    await expect(page.locator('.update-toast-text')).toHaveText('Update available: v1.0.0 → v1.1.0');
    await expect(page.locator('.update-toast-actions')).toBeVisible();
    await expect(page.locator('.update-toast button', { hasText: 'Update Now' })).toBeVisible();
  });
  ...
  test('updates text and hides actions after triggering update', async ({ page }) => {
    await page.locator('.update-toast button', { hasText: 'Update Now' }).click();

    await expect(page.locator('.update-toast-text')).toHaveText('Updated! Please restart the server manually.');
