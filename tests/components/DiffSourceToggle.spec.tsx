...
test.describe('DiffSourceToggle React component', () => {
  ...
  test('renders the toggle and updates the selected source', async ({ page }) => {
    const container = page.locator('#interactive-toggle-container .diff-source-toggle');
  ...
  test('applies radiogroup semantics', async ({ page }) => {
    ...
    await expect(group.locator('.toggle-option').nth(0)).toHaveAttribute('role', 'radio');
  });
});
