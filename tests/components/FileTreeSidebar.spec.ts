...
test.describe('FileTreeSidebar React component', () => {
  ...
  test('renders sidebar with title and file count', async ({ page }) => {
    ...
    await expect(sidebar.locator('.file-tree-sidebar__title')).toHaveText('Changed Files');
    await expect(sidebar.locator('.file-tree-sidebar__count')).toContainText('8 files');
  });
  ...
  test('shows empty state when no files', async ({ page }) => {
    ...
    await expect(page.locator('#file-tree-empty .file-tree-sidebar__count')).toHaveText('0 files');
  });
