...
test.describe('DataTable React component', () => {
  ...
  test('shows loading skeleton state', async ({ page }) => {
    ...
    await expect(page.locator('#loading-table .skeleton-line').first()).toBeVisible();
  });
  ...
  test('toggles group visibility on header click', async ({ page }) => {
    ...
    const initialRows = await page.locator('#grouped-table .data-table-row').count();
    await firstHeader.click();
    const newRows = await page.locator('#grouped-table .data-table-row').count();
