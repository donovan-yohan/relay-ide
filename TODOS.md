...
1. ~~**File browser:** File tree panel — navigate filesystem, preview files, open folders~~ _(done — PR #X, filesystem-browser-api)_
2. ~~**Changed file previews:** List changed files (git status/diff) with inline previews — see what an agent changed~~ _(done — PR #69, code-file-tools)_ **Note:** browser QA blocked by worktree PTY issue; needs `--preview` mode (see Small Features above) or manual QA from the main repo
...
4. **Open in external editor:** "Open in VS Code / Cursor" actions for repos, branches, worktrees — detect installed editors, quick-launch from context menu or command center _(low priority)_

### Verification-First Testing Strategy

Inspired by ["Cost to Implement vs Verify"](https://clifford.ressel.fyi/blog/cost-to-implement-vs-verify/) — as we lean harder on agents for implementation, our ability to _verify_ what they produce must scale proportionally. Today we have `npm test` (unit tests via `node --test`) and nothing else. The goal: full end-to-end verification infrastructure that agents can both run against and author tests for.
...
3. **Agent-authored test coverage:** Enable the coding agent (Claude/Codex) to write and run Playwright tests as part of its workflow:
   - Teach the agent (via CLAUDE.md or skill) to write a Playwright test _before_ or _after_ implementing a UI feature
