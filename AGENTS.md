...
Relay Agentic Development Environment — durable channel/DM conversations where humans and agents participate, backed by a hub/node web workbench, terminals, and repo-aware tools from any device. TypeScript + ESM backend (Express + node-pty + WebSocket) → `dist/`. React 19 frontend (Zustand + TanStack Query + Vite) → `dist/frontend/`.
...
| Channel chat         | `docs/CHANNEL_CHAT.md`                       | Shipped channel=conversation model: DMs, participants, routing, threads, images, mobile, legacy compatibility                                                                                    |
...
| ADRs                 | `docs/adrs/`                                 | Accepted decisions (latest: ADR-020 channel is the durable conversation)                                                                                                                         |
...
Channel = conversation; DM = channel; agent = participant. Provider sessions are replaceable execution bindings, not conversation identity. Use the six-layer vocabulary for wider IA work: View → Workspace → Project → Instance → Bench → Tab. A Workspace is a grouping/config layer, not a synonym for a repo; repo/worktree bindings are optional context. See `docs/CHANNEL_CHAT.md`, `docs/ARCHITECTURE.md`, and `docs/FRONTEND.md`.
...
- The hub owns durable channel history and mention routing. New channel/DM work targets `ChannelView` plus the channel router/store/hub; legacy `ChatView`/`Turn` is compatibility for restored/API-created web sessions until migration.
