...
export function useSessionHandlers({
  ...
  const navigateToSession = useCallback((sessionId: string, _sessionType: string) => {
  ...
  const handleOpenPrBranch = useCallback(async (pr: PullRequest, prPrompt?: string) => {
    const currentRepoPath = useUiStore.getState().activeRepoPath;
    const currentActiveWorkspace = currentRepoPath
      ? useSessionsStore.getState().repos.find((w) => w.path === currentRepoPath)
      : undefined;
  ...
  type PickerIntentHandler = (intent: SessionIntent, item: PickerItem) => Promise<void>;
