...
function ConnectedBody({
  ...
  return (
    <>
      <p className="github-integration-body-text">
        Connected as <strong>@{username ?? 'GitHub'}</strong>
      </p>
      {showConfirm ? (
        <>
          ...
          <div className="github-integration-action-row">
            <TuiButton variant="ghost" size="sm" onClick={onCancel}>
              Cancel
            </TuiButton>
            <TuiButton
              variant="danger"
        ...
        <TuiButton variant="danger" size="sm" onClick={onConfirm}>
          Disconnect
        </TuiButton>
      )}
    </>
  ...
  return (
    <div className="github-integration-device-flow">
      <div className="github-integration-code-row">
        <span className="github-integration-body-text">
          Enter code:{' '}
          <strong className="github-integration-user-code">
            {deviceCode.userCode}
          </strong>
        </span>
        <TuiButton variant="ghost" size="sm" onClick={onCopy}>
          Copy
        </TuiButton>
      </div>
      <p className="github-integration-body-text">
      ...
      <p className="github-integration-body-text github-integration-body-text--muted">
...
function useGitHubFlow(
  setGithubStatus: (s: GitHubStatus) => void,
    ...
    async function load() {
      try {
        ...
        setGithubStatus({
          ...
          ...(s.deviceFlowStatus
            ? { deviceFlowStatus: s.deviceFlowStatus }
            : {}),
        });
      } catch {
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (s.connected) {
            ...
            s.deviceFlowStatus === 'denied' ||
            s.deviceFlowStatus === 'expired'
          ) {
            setDeviceCode(null);
            setDeviceFlowError(
              s.deviceFlowStatus === 'denied'
...
export default function GitHubIntegration({
  ...
  const { deviceCode, deviceFlowError, disconnecting, connect, disconnect } =
    useGitHubFlow(setGithubStatus, setLoading);

  let statusText: string;
  if (loading) statusText = 'Checking connection...';
  else if (githubStatus?.connected)
    statusText = `Connected as @${githubStatus.username ?? 'GitHub'}`;
  else statusText = 'Not connected';

  function renderBody() {
    if (loading)
      return <CipherText loading={true} text="Checking connection..." />;
    if (needsReauth)
      return (
        <>
          <p className="github-integration-reauth-warning">
            Re-connect to enable webhook management
          </p>
          <TuiButton variant="primary" size="sm" onClick={() => void connect()}>
            Re-connect GitHub
          </TuiButton>
        </>
      );
    ...
    if (deviceCode)
      return (
        <DeviceFlowBody
          deviceCode={deviceCode}
          onCopy={() => void navigator.clipboard.writeText(deviceCode.userCode)}
        />
      );
    if (deviceFlowError)
      return (
        <>
          <p className="github-integration-error-text">{deviceFlowError}</p>
          <TuiButton variant="primary" size="sm" onClick={() => void connect()}>
            Try Again
          </TuiButton>
        </>
    ...
    return (
      <>
        <p className="github-integration-body-text github-integration-body-text--muted">
          Connect your GitHub account to enable PRs, CI status, and webhook
          management.
        </p>
        <TuiButton variant="primary" size="sm" onClick={() => void connect()}>
          Connect GitHub
