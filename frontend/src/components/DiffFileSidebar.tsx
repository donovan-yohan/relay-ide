...
export const DiffFileSidebar = forwardRef<DiffFileSidebarHandle, DiffFileSidebarProps>(
  function DiffFileSidebar({ files, activeFile, onSelectFile }, ref) {
    ...
    useImperativeHandle(ref, () => ({
      moveFocus(delta: number) {
        const next = Math.max(0, Math.min(files.length - 1, focusedIndexRef.current + delta));
