...
function useSendBuffer(dbg: (m: string) => void) {
  ...
  const flushSendBuffer = useCallback(() => {
    ...
    if (buf && isPtyConnected()) {
      const hex = Array.from(buf).map(c => c.charCodeAt(0).toString(16).padStart(2, '0')).join(' ');
      dbg('FLUSH: "' + buf.replace(/\x7f/g, '\u232b') + '" (' + buf.length + 'B) hex=[' + hex + ']');
      sendPtyData(buf);
    }
    sendBufferRef.current = '';
  }, [dbg]);

  const scheduleSend = useCallback((data: string) => {
...
function useMobileEffects(
  ...
  useEffect(() => {
    ...
    const onSelectionChange = () => {
      ...
      const len = inputEl.value.length;
      if (pos !== len && len > 0) dbg('SEL_DRIFT cursor=' + pos + ' len=' + len + ' → fixing');
...
function useKeydownHandler(
  ...
  return (e: React.KeyboardEvent<HTMLInputElement>) => {
    const inputEl = inputRef.current;
    if (!inputEl) return;
    dbg('KEYDOWN key="' + e.key + '" shift=' + e.shiftKey + ' composing=' + isComposingRef.current);
    if (!isPtyConnected()) return;
    let handled = true;
    switch (e.key) {
      case 'Enter': flushComposedText(); sendPtyData(e.shiftKey ? '\x1b[13;2u' : '\r'); inputEl.value = ''; break;
      case 'Backspace': if (inputEl.value.length === 0) sendPtyData('\x7f'); handled = false; break;
      case 'Escape': sendPtyData('\x1b'); inputEl.value = ''; break;
      case 'Tab': sendPtyData('\t'); break;
      case 'ArrowUp': sendPtyData('\x1b[A'); break;
      case 'ArrowDown': sendPtyData('\x1b[B'); break;
...
function useInputHandler(
  ...
  return (e: React.FormEvent<HTMLInputElement>) => {
    ...
    clearTimerRef.current = setTimeout(() => { dbg('TIMER_CLEAR'); inputEl.value = ''; ensureCursorAtEnd(inputEl, isComposingRef.current); }, 2000);
    if (!isPtyConnected() || isComposingRef.current) { if (isComposingRef.current) dbg('  skipped (composing)'); return; }
    const fallbackIntent = { type: ie.inputType, data: ie.data, rangeStart: null, rangeEnd: null, valueBefore: '', cursorBefore: 0 };
    ...
    if (result.newInputValue !== undefined) { inputEl.value = result.newInputValue; ensureCursorAtEnd(inputEl, isComposingRef.current); syncBuffer(inputEl, dbg); return; }
  ...
  return (
    <>
      <div className="debug-buttons">
        <button className="debug-toggle" style={{ opacity: dp.debugVisible ? 1 : 0.5 }} onClick={onToggle}>dbg</button>
        {dp.debugVisible && <><button className="debug-toggle" onClick={onCopy}>copy</button><button className="debug-toggle" onClick={onClear}>clear</button></>}
      </div>
      {dp.debugVisible && (
        <div className="debug-panel">
          <div className="input-mirror-inline">buf: &quot;{dp.mirrorValue}&quot; cursor={dp.mirrorCursor}</div>
          {dp.debugLines.map((line, i) => <div key={i}>{line}</div>)}
        </div>
      )}
    </>
  );
...
export const MobileInput = forwardRef<MobileInputHandle, Record<string, never>>(
  function MobileInput(_props, ref) {
    ...
    const flushComposedText = useCallback(() => {
      const inputEl = inputRef.current;
      if (!inputEl) return;
      if (isComposingRef.current && isPtyConnected() && inputEl.value) { dbg('FLUSH_COMPOSED: "' + inputEl.value + '"'); scheduleSend(inputEl.value); }
    ...
    useImperativeHandle(ref, () => ({
      ...
      focus: () => { dbg('FOCUS_REQ inputEl=' + (inputRef.current ? 'SET' : 'NULL')); inputRef.current?.focus(); },
      flushComposedText,
      clearInput: () => { if (inputRef.current) { inputRef.current.value = ''; inputRef.current.setSelectionRange(0, 0); } },
      onSessionChange: () => { isComposingRef.current = false; },
    }), [flushComposedText, dbg]);

    useMobileEffects(inputRef, isComposingRef, dbg, dp);

    const handleCompositionStart = (e: React.CompositionEvent<HTMLInputElement>) => { dbg('COMP_START data="' + e.data + '"'); isComposingRef.current = true; };
    const handleCompositionUpdate = (e: React.CompositionEvent<HTMLInputElement>) => { dbg('COMP_UPDATE data="' + e.data + '"'); };
    const handleCompositionEnd = (e: React.CompositionEvent<HTMLInputElement>) => {
      dbg('COMP_END data="' + e.data + '"');
      isComposingRef.current = false;
      if (isPtyConnected() && e.data) { dbg('  → COMP_SEND: "' + e.data + '"'); scheduleSend(e.data); }
    };
    ...
    const handleBeforeInput = (e: React.FormEvent<HTMLInputElement>) => {
      ...
      const inputEl = inputRef.current;
      if (!inputEl) return;
      if (ie.inputType.startsWith('delete') && inputEl.selectionStart === 0 && inputEl.selectionEnd === 0 && inputEl.value.length > 0) {
        dbg('CURSOR0_DEL type="' + ie.inputType + '"'); scheduleSend('\x7f'); e.preventDefault(); capturedIntentRef.current = null; return;
      }
      const ranges = ie.getTargetRanges?.() ?? [];
      const fr = ranges.length > 0 ? (ranges[0] as StaticRange) : null;
      capturedIntentRef.current = { type: ie.inputType, data: ie.data, rangeStart: fr ? fr.startOffset : null, rangeEnd: fr ? fr.endOffset : null, valueBefore: inputEl.value, cursorBefore: inputEl.selectionStart ?? 0 };
    };
    ...
    return (
      <>
        <form className="mobile-input-form" action="javascript:void(0)" onSubmit={handleFormSubmit}>
          <input ref={inputRef} type="search" className="mobile-input" dir="ltr" autoComplete="off" autoCorrect="on" autoCapitalize="sentences" spellCheck={true} enterKeyHint="send" aria-label="Terminal input"
            onCompositionStart={handleCompositionStart} onCompositionUpdate={handleCompositionUpdate} onCompositionEnd={handleCompositionEnd}
            onBlur={handleBlur} onBeforeInput={handleBeforeInput} onInput={handleInput} onKeyDown={handleKeydown} />
        </form>
        <DebugPanel dp={dp}
          onToggle={(e) => { e.preventDefault(); e.stopPropagation(); dp.setDebugVisible((v) => !v); }}
          onCopy={(e) => { e.preventDefault(); e.stopPropagation(); navigator.clipboard.writeText(dp.debugLines.join('\n')).then(() => dbg('--- LOGS COPIED ---')).catch(() => dbg('--- COPY FAILED ---')); }}
          onClear={(e) => { e.preventDefault(); e.stopPropagation(); dp.setDebugLines([]); }}
        />
      </>
    );
  }
