...
export const FileTree = forwardRef<FileTreeHandle, FileTreeProps>(
  function FileTree({ workspacePath }, ref) {
    ...
    // Fetch default branch once per workspace.
    const lastBranchFetchRef = useRef<string>('');
    useEffect(() => {
      if (!workspacePath || workspacePath === lastBranchFetchRef.current)
        return;
      lastBranchFetchRef.current = workspacePath;
      fetchDefaultBranch(workspacePath).then((b) => {
        setFileDiffDefaultBranch(b);
      });
    }, [workspacePath, setFileDiffDefaultBranch]);
    ...
    }, [queryClient, workspacePath, base]);
