...
function buildNeutralCandidates(
  ...
  addUnique(
    orderedPaths,
    seen,
    recentFiles.filter((t) => fileSet.has(t.filePath)).map((t) => t.filePath),
    MAX_NEUTRAL
  );
  addUnique(
    orderedPaths,
    seen,
    changedFiles.filter((p) => fileSet.has(p)),
    MAX_NEUTRAL
  );
  addUnique(orderedPaths, seen, allFiles, MAX_NEUTRAL);
  return orderedPaths.map((p) => ({
    path: p,
    result: { score: 0, matches: [] as [number, number][] },
  }));
}
...
function buildScoredResults(
  ...
  if (!q) {
    candidates = buildNeutralCandidates(
      fileSet,
      allFiles,
      recentFiles,
      changedFiles
    ...
    candidates = allFiles.flatMap((p) => {
      const r = scorePath(q, p);
      return r ? [{ path: p, result: r }] : [];
    });
...
function FileItem({
  ...
  return (
    <div
      className={['file-picker-item', isFocused ? 'focused' : '']
        .filter(Boolean)
        .join(' ')}
      role="option"
      tabIndex={-1}
    ...
    >
      <span
        className={['fp-item-cursor', isFocused ? 'visible' : '']
          .filter(Boolean)
          .join(' ')}
      >
        &gt;
      ...
      <span className="fp-item-filename">
        {highlightMatches(item.filename, item.result.matches, fileOffset).map(
          (seg, i) =>
            seg.highlight ? (
              <span key={i} className="fp-match">
                {seg.text}
              </span>
            ) : (
              seg.text
      ...
      <span className="fp-item-directory">
        {highlightMatches(item.directory, item.result.matches, dirOffset).map(
          (seg, i) =>
            seg.highlight ? (
              <span key={i} className="fp-match">
                {seg.text}
      ...
      {item.status && (
        <span
          ...
          style={{ color: statusToBadgeColor(item.status) }}
        >
...
function useMobileDrag(onClose: () => void) {
  ...
  const handleDragStart = useCallback(
    (e: React.TouchEvent, resultsScrollTop: number) => {
      if (!isMobileDevice) return;
      const target = e.target as HTMLElement;
      const isHandle =
        target.classList.contains('fp-drag-handle') ||
        target.classList.contains('fp-drag-bar');
...
function ResultsBody({
  ...
  if (isError)
    return <div className="file-picker-empty">failed to load files</div>;
  if (fetchError)
    return (
      <div className="file-picker-empty">no files found — {fetchError}</div>
    );
  ...
  if (flatItems.length === 0)
    return <div className="file-picker-empty">no files</div>;
  return (
    <>
      {sections.map((section, si) => {
        ...
        return (
          <React.Fragment key={section.label}>
            <div className="file-picker-section" role="presentation">
              {section.label}
            </div>
  ...
  if (truncated) {
    return (
      <div className="file-picker-footer">
        <span className="fp-hint truncated">
          showing {allFilesCount} of {total} files — type to filter
        </span>
      </div>
    );
  }
...
export function FilePicker({
  ...
  const handleTabKey = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      for (let i = sectionStarts.length - 1; i >= 0; i--) {
        if (focusedIndex >= (sectionStarts[i] ?? 0)) {
          currentSection = i;
          break;
        }
      }
  ...
  const handleBackdropClick = useCallback(
    (e: React.MouseEvent) => {
      if ((e.target as HTMLElement).classList.contains('file-picker-overlay'))
        onClose();
    },
    [onClose]
  );
  ...
  return (
    <div
      className={['file-picker-overlay', isMobileDevice ? 'mobile' : '']
        .filter(Boolean)
        .join(' ')}
      onClick={handleBackdropClick}
    >
      <div
        className={['file-picker', isMobileDevice ? 'mobile' : '']
          .filter(Boolean)
          .join(' ')}
        role="dialog"
        tabIndex={-1}
        ...
        onTouchStart={(e) =>
          handleDragStart(e, resultsRef.current?.scrollTop ?? 0)
        }
