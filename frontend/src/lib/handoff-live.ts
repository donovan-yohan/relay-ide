...
export function buildHandoffDraft(
  ...
  if (!session)
    return {
      draft: null,
      emptyReason: 'select an active tab before requesting a cold handoff plan',
    };
  const cwd = preferredCwd(session);
  if (!cwd)
    return { draft: null, emptyReason: 'active tab has no cwd to snapshot' };
  if (!session.workContextId)
    return { draft: null, emptyReason: 'active tab has no WorkContext yet' };
  ...
  const request: HandoffRequest = {
    ...
    source: {
      ...
      cwd,
      disposition:
        session.durability === 'stale-node' ? 'stale-source' : 'left-running',
    ...
    destination: {
      ...
      option: destination,
      cwd: destination.cwd,
      ...(destination.repoInstance?.repoInstanceId
        ? { repoInstanceId: destination.repoInstance.repoInstanceId }
        : {}),
      ...(destination.bench?.worktreeInstanceId
...
function handoffErrorViewFromEnvelope(
  ...
  const upstreamCode =
    typeof details?.['upstreamCode'] === 'string'
...
export function handoffStatusFromError(
  error: HandoffApiErrorView
): HandoffLiveStatus {
  if (error.code === 'CAPABILITY_DENIED') return 'capability-denied';
  if (error.code === 'SOURCE_STALE_OR_OFFLINE' || error.code === 'STALE_PLAN')
    return 'source-stale';
