...
// `createTerminalSession` launch path.
//
// Scope:
...
// `createTerminalSession` (which talks to the store + API) but does no
// store-state mutation of its own beyond what `createTerminalSession` already
...
export type LaunchEnvironmentResult =
  | { kind: 'launched'; result: CreateTerminalSessionResult }
...
/**
 * Map an EnvironmentOption to `createTerminalSession` options. Visible for
...
export function environmentToCreateSessionOptions(
  ...
  _overrides: LaunchEnvironmentOptions = {}
): CreateTerminalSessionOptions {
  const base: CreateTerminalSessionOptions = {
...
export async function launchEnvironment(
  ...
  createSession: (
    opts: CreateTerminalSessionOptions
  ) => Promise<CreateTerminalSessionResult> = createTerminalSession
